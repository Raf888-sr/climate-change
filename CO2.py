...
def get_temperature_data():
    temperature = pd.read_csv("https://www.ncdc.noaa.gov/cag/global/time-series/globe/land_ocean/1/12/1880-2021/data.csv")
    temperature.drop(temperature.iloc[:5].index,inplace=True)
    temperature = temperature.reset_index(drop=True)
    temperature.rename(columns={"Global Land and Ocean Temperature Anomalies":"Year"," December":"Temperature"},inplace=True)
...
fig.update_layout(template='plotly_dark',font_size=15)
...
selected_country = st.sidebar.multiselect('Select country or group',sorted_countries,['United States','EU-27','China','Russia','India'])
...
fig2.update_layout(template = 'plotly_dark',font_size=15,width=1500,height=600)
...
fig3 = px.line(co2_selected,
                x="year",
                y="co2",
                color="country",
                range_x=[selected_range[0],
                selected_range[1]+10],
                labels = dict(co2="CO2 emission in (tonnes)"))
fig3.update_layout(template='plotly_dark',hovermode="x unified",font_size=15,height=600,width=1500)
st.plotly_chart(fig3,use_container_width=True)

st.markdown("""*Source*: [OWID](https://ourworldindata.org/co2-and-other-greenhouse-gas-emissions)
and [NOAA](https://www.climate.gov/news-features/understanding-climate/climate-change-global-temperature)""")
